data = { response:
  {
    ...
    },
    {
      ...
      webTitle: "Margaret Atwood – You Have Been Warned!",
      review: "precious little of the woman herself",
...
existing = data[:response][:results]

data[:response][:results] = existing.map do |article|
  article.merge!({views: 0})
end

# pp data[:response][:results]

# ----

def read_article(data)
  index = rand(data[:response][:results].length)
  data[:response][:results][index][:views] += 1
  data
end
...
def display_views(data)
  data[:response][:results].each do |article|
